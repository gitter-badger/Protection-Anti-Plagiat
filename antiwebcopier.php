...
foreach ($bannav as $banned) {
    ...
    if($comparaison!==false) {
	    $write_this = 'information : ' . get_ip(); // Le texte que vous voulez avoir dans votre fichier antiwebcopier.txt
		$tentative++;
	}
}
if($tentative > 0){
	...
	echo utf8_decode( '[Sécurité] Notre site web est protégé contre le vole et le spam, vos information '.$navigateur.' sera automatiquement banni sur la base de donné projecthoneypot Bien essayé :D' ); // Le texte que vous voulez que le voleur recevra dans les fichiers télécharger
